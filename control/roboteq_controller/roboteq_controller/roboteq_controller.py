...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Frame, 'to_can_bus', 10)
        self.vel = Twist()
        self.cmd = Frame()
        self.diameter = 0.248
        self.tread = 0.6
    ...
    def callback(self,msg):
        ...
        # convert cg command
        r_ref = Decimal(str(r_ref)).quantize(Decimal("0"), rounding=ROUND_HALF_DOWN)
        l_ref = Decimal(str(l_ref)).quantize(Decimal("0"), rounding=ROUND_HALF_DOWN)
        r_byte = (int(r_ref)).to_bytes(4, "big", signed=True)  # high byteから順番に並んでいる
        l_byte = (int(l_ref)).to_bytes(4, "big", signed=True)
        r_cmd = [int(r_byte[0]), int(r_byte[1]), int(r_byte[2]), int(r_byte[3])]
        l_cmd = [int(l_byte[0]), int(l_byte[1]), int(l_byte[2]), int(l_byte[3])] 
#        self.cmd.header(
...
#        )
        ...
        self.cmd.id=0x210
        self.cmd.dlc= 8
        self.cmd.data=[l_cmd[3], l_cmd[2], l_cmd[1], l_cmd[0], r_cmd[3], r_cmd[2], r_cmd[1], r_cmd[0]]
