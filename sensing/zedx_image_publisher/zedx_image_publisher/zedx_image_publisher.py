...
class ZedXImagePublisher(Node):
    def __init__(self):
        ...
        # ROS2
        self.raw_img_pub_ = self.create_publisher(
            Image, 'pub_left_image', 1)
        self.commpressed_img_pub_ = self.create_publisher(
            CompressedImage, 'pub_left_compressed_image', 10)
        timer_raw = 1/30
    ...
    def image_callback(self):
        ...
        if self.err == sl.ERROR_CODE.SUCCESS:
            ...
            self.zed.retrieve_image(
                image_sl_left, sl.VIEW.LEFT, sl.MEM.CPU, self.image_size)
            image_cv_left = image_sl_left.get_data()
            pub_img = self.br.cv2_to_imgmsg(cv2.cvtColor(
                image_cv_left, cv2.COLOR_BGRA2BGR), 'bgr8')

            msg = CompressedImage()
            msg.format = "jpeg"
            ret, compressed_img = cv2.imencode(
                ".jpg", image_cv_left, [int(cv2.IMWRITE_JPEG_QUALITY), 30])
