...
class ZedXImagePublisher(Node):
    def __init__(self):
        ...
        timer_raw = 1/30   # 15 FPS
        timer_compressed = 1/30

        self.timer_image = self.create_timer(timer_raw, self.image_callback)
#        self.timer_compressed = self.create_timer(timer_compressed, self.compressed_callback)
    ...
    def image_callback(self):
        ...
        if self.err == sl.ERROR_CODE.SUCCESS :
            ...
            flag = self.zed.retrieve_image(image_sl_left, sl.VIEW.LEFT, sl.MEM.CPU, self.image_size)
#            ts = self.zed.get_timestamp(sl.TIME_REFERENCE.IMAGE)
#            print(f"flag = {flag}, timestamp = {ts.get_seconds()}.{ts.get_milliseconds()}")
            image_cv_left = image_sl_left.get_data()

#            image_sl_right = sl.Mat()
#            self.zed.retrieve_image(image_sl_right, sl.VIEW.RIGHT, sl.MEM.CPU, self.image_size)
#            image_cv_right = image_sl_right.get_data()
            
#            sbs_image = np.concatenate((image_cv_left, image_cv_right), axis=1)

            pub_img = self.br.cv2_to_imgmsg(image_cv_left, 'bgra8') 
            pub_img.header.stamp = self.get_clock().now().to_msg()
            self.raw_img_pub_.publish(pub_img)
 
    def compressed_callback(self):
        self.err = self.zed.grab(self.runtime)
        if self.err == sl.ERROR_CODE.SUCCESS :
            image_sl_left = sl.Mat()
