...
class ObjectEstimation:
    def __init__(self, node: Node):
        ...
        PedestrianRisk_X_u = self.object_risk_table_x_u
        PedestrianRisk_X_y = self.object_risk_table_x_y
        PedestrianRisk_Y_u = self.object_risk_table_y_u
        PedestrianRisk_Y_y = self.object_risk_table_y_y
    ...
    def init_parameters(self, node: Node):
        self.object_risk_table_x_u = get_ros_parameter(
            node, "object_risk_potential.pedestrian_risk_table_x_u")
        self.object_risk_table_x_y = get_ros_parameter(
            node, "object_risk_potential.pedestrian_risk_table_x_y")
        self.object_risk_table_y_u = get_ros_parameter(
            node, "object_risk_potential.pedestrian_risk_table_y_u")
        self.object_risk_table_y_y = get_ros_parameter(
            node, "object_risk_potential.pedestrian_risk_table_y_y")
    ...
    def calculation_object_risk(self, objs, seek_position_absolute):
        if len(objs) == 0:
            ...
            seek_point1_risk = self.object_risk(objs, seek_position_absolute[0])
            # list num x (5x1), np.array 2x5
            seek_point2_risk = self.object_risk(objs, seek_position_absolute[1])
            # list num x (5x1), np.array 2x5
            seek_point3_risk = self.object_risk(objs, seek_position_absolute[2])
    ...
    def object_risk(self, objs, seek_position_absolute):
        ...
        for turb in range(seek_position_absolute.shape[1]):
            sum_risk = 0
            for obj in objs:
                # risk_potential by array_function
                #u = obj[0:2] - seek_position_absolute[:, turb]
                ...
                risk_value = self.bivariate_gaussian(obj[0],obj[1],seek_position_absolute[:, turb][0],seek_position_absolute[:, turb][1],0.8,0.8,0.0)
                sum_risk += risk_value * self.object_risk_gain * obj[2] * (obj[3] ** 2)
    ...
    def bivariate_gaussian(self, object_x, object_y, seekpoint_x, seekpoint_y, sigma_x, sigma_y, coefficient):
        ...
        exponent = -1 / (2 * (1 - coefficient**2)) * (
            (object_x - seekpoint_x)**2 / sigma_x**2 - 2 * coefficient * (object_x - seekpoint_x) * (object_y - seekpoint_y) / (sigma_x * sigma_y) + (object_y - seekpoint_y)**2 / sigma_y**2
        )
