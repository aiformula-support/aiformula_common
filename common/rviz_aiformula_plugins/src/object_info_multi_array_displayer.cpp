...
void ObjectInfoMultiArrayDisplayer::displayObjectWidth(
    ...
    for (const auto &object : objects) {
        ...
        Ogre::ColourValue color = rviz_common::properties::qtToOgre(width_color_property_->getColor());
        shape.setColor(getColorWithConfidence(color, object.confidence));
    }
}
...
void ObjectInfoMultiArrayDisplayer::displayObjectId(const std::vector<aiformula_interfaces::msg::ObjectInfo> &objects) {
    ...
    for (const auto &object : objects) {
        auto *text =
            id_texts_.emplace_back(std::make_shared<rviz_rendering::MovableText>(std::to_string(object.id))).get();
        text->setTextAlignment(rviz_rendering::MovableText::H_CENTER, rviz_rendering::MovableText::V_CENTER);
        text->setCharacterHeight(1.0);
        text->setColor(rviz_common::properties::qtToOgre(id_color_property_->getColor()));
...
Ogre::ColourValue ObjectInfoMultiArrayDisplayer::getColorWithConfidence(Ogre::ColourValue color,
